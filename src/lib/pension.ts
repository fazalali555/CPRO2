 ...
 * - The age factor is bounded by the available factor table
 * 
...
export const resolveAgeFactor = (age?: number): { age: number; factor: number } => {
  // Get the age, default to 60 if not provided
  ...
  const minAge = Math.min(...factorAges);
  const maxAge = Math.max(...factorAges);
...
export const calculatePension = (params: PensionParams): PensionResult => {
  if (params.basicPay == null || params.personalPay == null || params.qualifyingServiceYears == null) {
  ...
  // Age is already expected to have the 6-month rule applied from the caller
  // and will be capped at 60 inside resolveAgeFactor
  ...
  // Medical Allowance
  // 25% of Net Pension (Base)
  const medicalAllowance2010 = round2(netPension * 0.25);
  // 25% Increase on Medical Allowance (2022)
...
export const calculatePensionSafe = (params: PensionParams): { ok: true; value: PensionResult } | { ok: false; error: string } => {
  try {
...
export const calculateFamilyPension = (
  ...
  // 4. Family Pension Base
  let familyPensionBase = 0;
  if (isDeathAfterRetirement) {
    familyPensionBase = netPension * 0.75;
  } else {
    // Death In Service: usually 50% of Gross
    familyPensionBase = grossPension * 0.50;
  }

  // 5. Calculate Increases (Compounding, post-2022 only for current scales)
  ...
  // 6. Medical Allowance
  const maBase = round2(familyPensionBase * 0.25);
