...
export const AIImportModal: React.FC<AIImportModalProps> = ({ onClose, onImport }) => {
  ...
  const handleConfirm = () => {
    ...
    let bodyHtml = parsed.body || parsed.bodyHtml;
    
    // 1. Detect and Convert Markdown Tables
    const tableRegex = /^\|(.+)\|$\n^\|([-:| ]+)\|$\n((?:^\|(.+)\|$\n?)+)/gm;
    bodyHtml = bodyHtml.replace(tableRegex, (match: string, header: string, divider: string, rows: string) => {
      ...
      rowLines.forEach((line: string) => {
        ...
        html += '<tr>';
        cols.forEach((c: string) => html += `<td style="border: 1px solid #d1d5db; padding: 8px 12px;">${c}</td>`);
        html += '</tr>';
      });
    ...
    bodyHtml = bodyHtml.replace(pseudoTableRegex, (match: string) => {
      const cols = match.split(/\t|\s{3,}/).map((c: string) => c.trim()).filter(Boolean);
      ...
      cols.forEach((c: string) => html += `<td style="border: 1px solid #d1d5db; padding: 6px 10px;">${c}</td>`);
      html += '</tr></table>';
      return html;
    });
