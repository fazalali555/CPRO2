...
import { EditorProvider, useEditorInstance } from "@/contexts/EditorContext";
...
const EditorStateBridge: React.FC<{ value: string }> = ({ value }) => {
  ...
  const loadContent = (content: string) => editor?.commands.setContent(content);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <Card variant="elevated" className="bg-surface p-6 shadow-lg h-fit sticky top-6">
          ...
          <div className="space-y-4 text-sm">
            ...
            {urgency !== 'routine' && (
              <div>
                <Badge label={URGENCY_LEVELS.find(u => u.value === urgency)?.label || urgency} color="tertiary" />
              </div>
            )}
          ...
          <div className="mt-6 pt-4 border-t border-outline/20">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {printSettings.includeWatermark && (
                <Badge label={`WM: ${printSettings.watermarkText || 'DRAFT'}`} color="secondary" className="text-xs" />
              )}
            </div>
          </div>
        </Card>
