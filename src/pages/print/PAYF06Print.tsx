...
export const PAYF06Print: React.FC<Props> = ({ employees, cases }) => {
  ...
  useEffect(() => {
    if (caseRecord && employee) {
      document.title = `PAYF06 - ${employee.employees.name}`;
    }
  }, [caseRecord, employee]);

  const isValidType = caseRecord?.case_type === 'gpf_non_refundable' || caseRecord?.case_type === 'gpf_final';
  ...
  if (!isValidType) {
    return (
      <div className="p-10 text-center">
        ...
        <p>This form (PAYF06) is only for GPF Non-Refundable or GPF Final cases.</p>
