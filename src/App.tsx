...
const App: React.FC = () => {
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('clerk_pro_theme') === 'dark');

  // Set theme on mount and when darkMode changes
  useEffect(() => {
    localStorage.setItem('clerk_pro_theme', darkMode ? 'dark' : 'light');
    if (darkMode) {
  ...
  useEffect(() => {
    try {
      const enabled = localStorage.getItem('clerk_pro_auto_backup_enabled') === 'true';
      if (!enabled) return;

      const last = localStorage.getItem('clerk_pro_auto_backup_at');
      const lastTime = last ? new Date(last).getTime() : 0;
      const twentyFourHours = 24 * 60 * 60 * 1000;

      if (!last || Date.now() - lastTime > twentyFourHours) {
        BackupService.createAutoSnapshot().catch((error) => {
